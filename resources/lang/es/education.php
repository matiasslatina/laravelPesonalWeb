<?php

return[
    'education'=>'formación',
    'educations' =>
    [
        [
            'institute'=>'UNOBA - Universidad Del Noroeste',
            'curse_name'=>'Programador Universitario',
            'description'=>'90% de la carea aprobada.',
            'date'=>'2007 - 2013'
        ],

    ],
    'certifications' =>
    [
        [
            'title'=>'Desarrollo web en HTML 5, CSS3 y Javascript (nivel avanzado)',
            'place'=>'UTN - Uiversidad Tecnológica Nacional',
            'description'=>'APIs y elementos avanzados de HTML5, Css3 y Javascript para desarrollos web o mobile',
            'date'=>'Dic 2019 - Feb 2020'
        ],
        [
            'title'=>'Desarrollo en React JS',
            'place'=>'UTN - Uiversidad Tecnológica Nacional',
            'description'=>'Aplicación y adaptación de los conocimientos en programación web al desarrollo de aplicaciones SPA explotando las ventajas brindadas por Reac en cuanto al flujo de datos.',
            'date'=>'Feb 2020 - May 2020'
        ]
    ],
]

?>
